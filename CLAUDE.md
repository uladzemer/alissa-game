...
- Глубина (от дальнего к ближнему): `bg-*` (размыт и притуманен при сборке, 0.12) → `mid-*` (0.4) → `near-*` (0.7) → игра (1.0) → `fg-*` (размытые растения/камни, 1.35). Частицы, лучи, виньетка и тень Алисы — `setupAtmosphere`/`updateLayers` в `Level.ts`; на слабых телефонах (`LOW_POWER`) частиц вдвое меньше.
- Статичные декорации уровня прячутся вне экрана (`statics`, проверка раз в 5 кадров) — рисуется только видимое.
- Картинки НИКОГДА не растягивать непропорционально: только `setScale` одним числом, TileSprite с `setTileScale` или 3/9-slice (`src/game/ui.ts`: нарисованные кнопки, панели, ленты, круглые плашки). Титул/финал обрезаются (`cover_crop`, 1280×800 с запасом высоты), а не сжимаются; средний слой леса растворяется в тумане сверху (`fade`). Подписи не ставить на лица иллюстраций.
- Звуки: исходники `art/sfx/*.mp3` (Kenney, CC0; шаги — синтез «тук-тук» в `sfx.ts`, выбран владельцем с дочкой на странице прослушивания) упакованы сборщиком в один `public/sfx.json` (MP3-запросы перехватывают расширения-загрузчики и сохраняют файлами), грузится после первого нажатия, при сбое — синтез из `sfx.ts`. Музыка пока синтез (промпты для настоящей — `docs/art-and-audio-prompts.md`). Playwright-браузер — Chrome владельца с расширением-загрузчиком: прямые .mp3 он скачивал в D:!Downloads.
- Производительность: `LOW_POWER` (слабые телефоны) — меньше частиц; `QUALITY.lite` включается сам, если fps < 26 четыре секунды подряд. В тестах `window.__quality.locked = true` (Playwright рендерит ~31 fps).
- Враги не погибают: «добреют» и улетают (правило владельца, без жестокости).

## Картинки
- Параллельные прогоны Codex на одном аккаунте путают файлы (берут «самый новый PNG» чужого прогона) — в промпт: брать путь, который вернул СВОЙ вызов, открыть картинку для проверки; результат смотреть глазами (контакт-лист).
