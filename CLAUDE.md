...
- Звуки: `public/sfx/*.mp3` (Kenney, CC0, лицензия рядом), грузятся после первого нажатия, при сбое — синтез из `sfx.ts`. Музыка пока синтез (промпты для настоящей — `docs/art-and-audio-prompts.md`). В тестовом браузере Playwright MP3 приходят пустыми (204) — это особенность теста, curl отдаёт 200.
